...
(defn materialize-git-deps
  ...
  jars are reused. Bazel *target* names stay name-based (stable across sha
  bumps); only the jar path attribute changes when the sha changes.

  Only classpath entries that tools.deps tagged with `:lib-name` are considered.
  Project `:paths` (workspace source roots) are not lib-backed and are never
  packed. Directory libs without `:git/sha` (e.g. `:local/root`) are rejected."
    ...
    (reduce
     (fn [basis [lib dirs]]
       (let [coord (get-in basis [:libs lib])
             sha (:git/sha coord)
             _ (throw-if-not! sha
                              (str "unsupported dependency type for " lib ": "
                                   "directory classpath entries require a git dep with :git/sha "
                                   "(:local/root and other non-git directory deps are not supported)")
