...
When bazel sets `XML_OUTPUT_FILE` (it does for every test action), the runner also writes a JUnit XML report there, with one `<testcase>` per `deftest` (including per-test timing) and `<failure>`/`<error>` detail. This lets bazel surface structured, per-test results instead of synthesizing a bare pass/fail from the exit code. A namespace that fails to load is reported as a single errored `load` testcase; an exception that escapes the run (e.g. from a `:once`/`:each` fixture) is reported as a `fixture-error` testcase after any tests that already ran.
