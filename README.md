...
## tools.deps / Maven dependencies (optional)

There are two ways to turn a Maven-only `deps.edn` into Bazel jars under `@deps//:…`
(same label munging as `rules_jvm_external`, e.g. `@deps//:org_clojure_clojure`).

### Preferred: Bazel downloads via rules_jvm_external

**Pin** (tools.deps resolves the graph once; may use a local m2 cache only for this step):
...
Commit `deps_install_lock.json`. Wire **rules_jvm_external** in `MODULE.bazel` so **Bazel**
downloads the jars (no `$HOME/.m2` symlink at fetch time):
...
maven.install(
    name = "deps",
    artifacts = [
        # copy GAVs from pin output (deps_artifacts.bzl) after each pin
...
See `examples/simple` for a working setup. Pin **rejects** non-Maven coords (git / `:local/root`)
for now; those may return later as additional procurers without undoing this path.

### Legacy: tools.deps downloads into ~/.m2

In your WORKSPACE / older bzlmod `deps.install` flow:
...
This still uses tools.deps to download into a host Maven cache (symlinked into `@deps`). Prefer
the rules_jvm_external path above for hermetic fetch / RBE.

With the legacy path, for each Clojure namespace in a dependency jar an additional AOT
`clojure_library` target may be generated (`@deps//:ns_…`). Generated `gen_srcs` libraries
depend on those when needed.

The rje path exposes **jar** targets (`@deps//:org_clojure_clojure`, …). Hand-written
`clojure_library` deps on those jars work today (`examples/simple`). **Per-ns AOT targets
on top of rje** (for gen_srcs-heavy graphs that reference `@deps//:ns_…`) are a follow-up:
pin already writes `deps_aot_manifest.edn` as input for that layer. Git / `:local/root`
remain possible later as extra procurers; this path only changes Maven download.
