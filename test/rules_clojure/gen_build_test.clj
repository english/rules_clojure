...
(deftest pack-dirs-includes-file-symlink-content
  (testing "a file symlink is packed as a regular entry with the target's content"
    (let [tmp (fs/new-temp-dir "pack-dirs-filesym-test")
          src (fs/->path tmp "src")]
      (try
        (write-tree! tmp {"src/foo/real.clj" "(ns foo.real)"})
        (java.nio.file.Files/createSymbolicLink
         (fs/->path src "foo" "link.clj")
         (fs/->path src "foo" "real.clj")
         (into-array java.nio.file.attribute.FileAttribute []))
        (let [jar (fs/->path tmp "out.jar")]
          (jar/pack-dirs! jar [src])
          (is (contains? (set (jar-entry-names jar)) "foo/link.clj"))
          (is (= "(ns foo.real)" (jar-entry-content jar "foo/link.clj"))))
        (finally
          (fs/rm-rf tmp))))))
  ...
  (let [tmp (fs/new-temp-dir "materialize-local-test")
    ...
    (try
      ...
      (let [basis {:classpath {local-dir {:lib-name lib}}
        ...
        (is (thrown-with-msg? Exception #":local/root and other non-git directory deps are not supported"
                              (gb/materialize-git-deps basis build-dir))))
      (finally
        (fs/rm-rf tmp)))))

(deftest materialize-git-deps-skips-project-paths-without-lib-name
  "Workspace :paths appear as directory classpath roots without :lib-name and
   must not be packed into gitjars/."
  (let [tmp (fs/new-temp-dir "materialize-paths-test")
        project-src (fs/->path tmp "project" "src")
        build-dir (fs/->path tmp "deps-build")]
    (try
      (write-tree! tmp {"project/src/app/core.clj" "(ns app.core)"})
      (fs/ensure-directory build-dir)
      (let [basis {:classpath {project-src {}}
                   :classpath-roots [project-src]
                   :libs {}}
            basis' (gb/materialize-git-deps basis build-dir)]
        (is (= basis basis') "basis unchanged when no lib-backed dirs")
        (is (not (fs/exists? (fs/->path build-dir "gitjars")))
            "no gitjars directory is created for project paths"))
      (finally
        (fs/rm-rf tmp)))))
